...
class PyBullet:
    ...
    def create_box(
        ...
        ) -> int:  # Modify return type to int
            """Create a box and return its unique ID.
    ...
    def getContactPoints(bodyA, bodyB):
        return p.getContactPoints(bodyA,bodyB)
    
    def getNumBodies():
        for i in range(p.getNumBodies()):
            body_info = p.getBodyInfo(i)
            body_name = body_info[1].decode("utf-8")  # body_info[1] contains the body name
            print(f"Body ID: {i}, Body Name: {body_name}")